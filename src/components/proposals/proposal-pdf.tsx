...
function generateProposalPDF({ proposal, projectName, companyName, companyEmail, companyPhone }: Props) {
  ...
  // ── RIGHT: PROPOSAL header ────────────────────────────────────────────────
  doc.setFontSize(26);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...primary);
  doc.text("PROPOSAL / QUOTE", pageWidth - margin, 52, { align: "right" });
  ...
  doc.text("This proposal was generated by WorkSupp", pageWidth / 2, pageHeight - 30, { align: "center" });

  const slug = projectName.replace(/[^a-z0-9]/gi, "-").toLowerCase().slice(0, 40);
  doc.save(`proposal-${slug}.pdf`);
}

export function ProposalDownloadButton({ proposal, projectName, companyName, companyEmail, companyPhone }: Props) {
  function handleClick() {
    try {
      generateProposalPDF({ proposal, projectName, companyName, companyEmail, companyPhone });
    } catch (e) {
